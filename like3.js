...
function touchmestart ( targetElement ) {
 try {
...
function touchmeend ( targetElement ) {
 try {
    //var targetElement = document.activeElement;
    console.log(targetElement);
    var evt = document.createEvent('UIEvent');
...
function click_link ( links, period, name ) 
{
    ...
    try { touchmestart (links[0]); touchmeend (links[0]); links[0]. click(); } catch(err) { alert (err); }
...
function happyFn ( happy, period ) 
{
    ...
    tc++;
    try { touchmestart(happy[0]); touchmeend(happy[0]); happy[0]. click(); } catch(err) { alert (err); }
